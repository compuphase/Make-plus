...
static int default_warn_undef_vars = 0;
...
find_and_set_default_shell (const char *token)
{
  ...
  if (((tokend == search_token
        || (tokend > search_token
            && (tokend[-1] == '/' || tokend[-1] == DIRSEP_C)))
       && !strcasecmp (tokend, "cmd"))
      || ((tokend - 4 == search_token
           || (tokend - 4 > search_token
               && (tokend[-5] == '/' || tokend[-5] == DIRSEP_C)))
...
# endif
      ...
      if (program)
        {
          ...
          if (argv0_len > 4 && streq (&program[argv0_len - 4], ".exe"))
            program[argv0_len - 4] = '\0';    /* Remove .exe extension, if present. */
        }
#elif defined(VMS)
     ...
     some compilers (MSVC) don't like conditionals in macros.  */
  {
    const char *features = "target-specific order-only second-expansion"
                           " else-if ifset shortest-stem undefine oneshell"
                           " runtime-macros .path"
#ifndef NO_ARCHIVES
                           " archives"
#endif
        ...
        if (len == 13 && strneq (envp[i], "MAKE_RESTARTS", 13))
          {
            ...
            restarts = (unsigned int) strtoul (ep, NULL, 10);
            export = v_noexport;
          }
  ...
  /* Decode the switches from the environment.  */
  decode_env_switches (STRING_SIZE_TUPLE ("GNUMAKEFLAGS"));

  /* Clear GNUMAKEFLAGS to avoid duplication.  */
  if (lookup_variable (STRING_SIZE_TUPLE ("GNUMAKEFLAGS")))
  ...
  config_file_path = read_config(config_file_path, explicit_config, argv[0]);  /* argv[0] is only used in DOS compile. */
  {
    ...
    if (opts && strlen (opts) > 0)
      {
        decode_string_switches (opts);
        *opts = '\0';   /* Clear GNUMAKEFLAGS in the configuration data */
      }
  }
  ...
  /* Decode the switches on the command line. */
  {
    int env_slots = arg_job_slots;  /* Remember the job slots set through the environment vs. command line. */
  ...
  /* Figure out the level of recursion.  */
  {
    ...
    if (v && v->value[0] != '\0' && v->value[0] != '-')
      makelevel = (unsigned int) strtoul (v->value, NULL, 10);
    else
      makelevel = 0;
  }
...
# ifdef __EMX__
      ...
      && (strchr (argv[0], '/') != 0 || strchr (argv[0], DIRSEP_C) != 0)
# endif
      )
  ...
  if (current_directory[0] != '\0'
      ...
      && (argv[0][0] != DIRSEP_C && (!argv[0][0] || argv[0][1] != ':'))
      && strchr (argv[0], DIRSEP_C) != 0
#endif
      )
    argv[0] = xstrdup (concat (3, current_directory, "/", argv[0]));
#endif /* !__MSDOS__ */
              ...
              if (d->file->unloaded)
                {
                  ...
                  if (load_file (&d->floc, &filename, 0) == 0)
...
/* Decode switches from a zero-terminated string. We do this by chopping the
   string into a vector of words, prepending a dash to the first word if it
   lacks one, and passing the vector to decode_switches.  */

static void
decode_string_switches (const char *options)
{
  char *p, *buf;
  int len, argc;
  const char **argv;
  ...
  /* We need a buffer to copy the string into while we split it into words
  ...
  while (*options != '\0')
    {
      if (*options == '\\' && options[1] != '\0')
        ++options;                /* Skip the backslash.  */
      else if (ISBLANK (*options))
        {
          /* End of the word.  */
          *p++ = '\0';
          argv[++argc] = p;
...
/* Decode switches from variable ENVAR of LEN characters long (which is
...
print_version (void)
{
  ...
  printf ("%sCopyright (C) 1988-2016 Free Software Foundation, Inc.\n"
          "%sPortions copyright (c) 2019-2025 CompuPhase\n",
          precede, precede);
